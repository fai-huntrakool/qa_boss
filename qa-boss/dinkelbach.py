...
def dinkelbach_for_multiple_ratios(x, lamb, v, u, vk, uk, numerator, divisor, previous_solution, i=1):
    bqm = gtp.construct_bqm(x, lamb, numerator, divisor)
    current_solution = solver.sa_solver(bqm, previous_solution)
    ...
    if obj_2 != 0:
        lamb = update_lambda(lamb, u, v, sub_obj_1, sub_obj_2)
        dinkelbach_for_one_ratio(x, lamb, numerator, divisor, previous_solution, i + 1)
    elif obj_2 == 0 | i == 50:
        return x, obj_1, obj_2


def initialize_lambda(size, numerator, divisor, vk, uk):
    while True:
    ...
    u = uk
    lamb = (v + u) / 2
    return lamb, v, u, vk, uk, x_0


def update_lambda(lamb, u, v, sub_obj_1, sub_obj_2):
    uk = []
    vk = []
    if min(sub_obj_2) < 0:
